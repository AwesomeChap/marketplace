...
const SellerDashBoard = (props) => {
  ...
  const diningPossible = () => {
    const branchIndex = props.sellerConfig.branches.map(obj => obj._id).indexOf(props.branchId);
    return !!props.branchId && !!props.sellerConfig.branches && props.sellerConfig.branches[branchIndex].profile.serviceOptions.includes("Dining In");
  }
  ...
  const TabPanes = {
    ...
    seatArrangement: <SeatArrangement diningPossible={diningPossible} done={() => setLoading(false)} loading={loading} handleSaveConfig={handleSaveConfig} sellerConfig={props.sellerConfig} branchId={props.branchId} />,
    order: <div>Order</div>,
  }
        ...
        <Tabs destroyInactiveTabPane={true} onChange={handleChange} activeKey={currentKey} tabBarExtraContent={operations}>
          {
            Object.keys(TabPanes).map((key) => {
              return (
                <TabPane disabled={(tabHeaders.includes(key) && (newConfig || !props.sellerConfig.branches || !props.sellerConfig.branches.length)) || (key === "seatArrangement" && !diningPossible())} tab={_.startCase(key)} key={key}>
                  {TabPanes[key]}
                </TabPane>
              )
            })
