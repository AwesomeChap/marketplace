...
router.delete('/', (req, res) => {
    if (!req.body.userId) {
        ...
        User.findById(req.body.userId).then((user) => {
            ...
            Advertisement.findOneAndDelete({ _userId: user._id }).then(advt => {
                if (!advt) {
                    return res.status(404).json({ message: "Advertisement not found" });
                }
                console.log(advt._id);
                switch (advt.duration.key) {
                    ...
                    case 12: AdvtStatusToken12.findOAndDeletene({ _advtId: advt._id }).then(advtToken => {
                        if (advtToken) {
                            return res.status(200).json({ message: "Advertisement plan terminated" });
                        }
                    }).catch(e => res.status(500).json({ errors: e })); break;
